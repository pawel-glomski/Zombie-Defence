...
void NormalZombie::move(Vector2f targetPosition)
{
	if (!isKnockbacked)
	{
		...
		if (normalZombieMovment.getWalkingSprite().getGlobalBounds().contains(targetPosition))				// nie pozwala zombie wejsc na gracza
		{
			normalZombieMovment.getWalkingSprite().move((normalZombieMovment.getWalkingSprite().getPosition().x - targetPosition.x) / 100.0,
				(normalZombieMovment.getWalkingSprite().getPosition().y - targetPosition.y) / 100.0);
		}
	}
	else
	{
...
void NormalZombie::moveWalkingSprite(Vector2f vector)
{
	normalZombieMovment.getWalkingSprite().move(vector);
}
